...
public class Calculator extends JFrame implements ActionListener, ComponentListener{
	...
	private JTextArea[] display = new JTextArea[4];
	...
	private void constructCalc()
	{
		...
		{
			JTextArea toAdd = new JTextArea("0");
		...
		this.setSize((int) (width * .3175), (int) height * 3 / 4);////////////////////////////////////////////////////////////////////////LOCKING SCREEN SIZE/////////////////////
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if (e.getSource().equals(botP[4]))
		{
			...
			else
			{
				...
				if (change[0].isSelected())
				{
					baseText[1].setText(hexConverter(text.getText(), 10));
					Long y = Long.parseLong(baseText[1].getText());
					baseText[0].setText(Long.toHexString(y).toUpperCase());
					baseText[2].setText(Long.toOctalString(y));
					baseText[3].setText(Long.toBinaryString(y));
				...
				{
					baseText[1].setText(octConverter(text.getText()));
					Long y = Long.parseLong(baseText[1].getText());
					baseText[0].setText(Long.toHexString(y).toUpperCase());
					baseText[2].setText(Long.toOctalString(y));
					baseText[3].setText(Long.toBinaryString(y));
				...
				{
					baseText[1].setText(binConverter(text.getText()));
					Long y = Long.parseLong(baseText[1].getText());
					baseText[0].setText(Long.toHexString(y).toUpperCase());
					baseText[2].setText(Long.toOctalString(y));
					baseText[3].setText(Long.toBinaryString(y));
		...
		else if (e.getSource().equals(botP[29]))
		{
			...
			{
				result = evaluate(eval); // Evaluate fully
			}
			equation.setText("  ");
			hEquation = "";
			dEquation = "";
	 ...
	 */
	private String hexConverter(String hex, int radix)
	{
		int sum = 0;
		int toAdd;
		char at = ' ';
	 ...
	 */
	private String octConverter(String oct)
	{
		int sum = 0; 
		int toAdd; 
		int mult = 0;
	 ...
	 */
	private String binConverter(String bin)
	{
		int sum = 0; 
		int toAdd; 
		int mult = 0;
	...
	private String evaluate(String input)
	{
		...
		while (scan2.hasNext())
		{
			...
			{
				int op1 = operands.pop().intValue();
				int op2 = operands.pop().intValue();
				Integer toAdd;
